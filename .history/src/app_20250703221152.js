...
const encuestaFlow = addKeyword(afirmaciones)
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    if (paso >= preguntas.length) {
        // Obtener el idContacto basado en ctx.from (número de teléfono)
    const pool = await mssql.connect()
    const result = await pool.request()
      .input('num', sql.NVarChar(50), ctx.from)  // ctx.from es el número de teléfono del usuario
      .query(`
        SELECT id
        FROM contactos
        WHERE num = @num
      ...
      await flowDynamic('❌ No se encontró un contacto asociado con este número.');
      return;
    }

    const idContacto = result.recordset[0].id;
