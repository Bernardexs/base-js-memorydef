...
const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    // Extraemos idEncuesta e idEmpresa del usuario
    const idEncuesta = usuario.idEncuesta;
    const idEmpresa  = usuario.idEmpresa; // ← nunca será null

    // 3) Detectar cambio de encuesta y resetear si cambió
    const prevEncuesta = await state.get('idEncuesta');
    if (prevEncuesta && prevEncuesta !== idEncuesta) {
      await state.update({
        finalizada: false,
        preguntas:  null,
        respuestas: [],
    ...
    // 4) Guardar en memoria idEncuesta e idEmpresa
    await state.update({ idEncuesta, idEmpresa });
    ...
    // 6) Si ya completó, avisar y salir
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 7) Si no está aún en progreso, marcamos como “en progreso”
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta,
        idEmpresa  // ← uso directo de usuario.idEmpresa
      });
      // Backend interpretará esta primera llamada como “en progreso”
    }

    // 8) Si ya cargamos preguntas en memoria, no hacemos nada más
    if (await state.get('preguntas')) {
      return;
    }

    // 9) Inicializo estado local y envío la primera pregunta
    await state.update({
      preguntas,
      respuestas: [],
      paso:       0,
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    // 10) Limpiar recordatorio actual
    clearReminder(ctx.from);
    ...
    // 11) Validaciones según tipo de respuesta
    if (preguntaActual.tipoRespuesta === 'RANGO') {
    ...
    // 12) Guardar respuesta y avanzar paso
    respuestas.push(respuesta);
    paso++;

    // 13) Si quedan preguntas, envío la siguiente
    if (paso < preguntas.length) {
    ...
    // 14) Última respuesta: limpio el state
    await state.update({ finalizada: true, preguntas: null, respuestas: [], paso: null });
    ...
    const payload = preguntas.map((p, i) => ({
      ...
      idEmpresa:  p.idEmpresa, // viene en cada pregunta
      pregunta:   p.pregunta,
    ...
    try {
      // a) Guardar respuestas
      await axios.post('http://localhost:7003/guardar-respuestas', payload);
      await flowDynamic('📩 Tus respuestas fueron enviadas exitosamente.');

      // b) Marcar completada (segunda llamada)
      const idEncuesta = preguntas[0].idEncuesta;
      const idEmpresa  = preguntas[0].idEmpresa;
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta,
        idEmpresa
      ...
      console.error('Error al guardar o marcar completada:', e.message);
      await flowDynamic('⚠ Hubo un problema al procesar tu encuesta. Intenta más tarde.');
    }

    // 16) Despedida y resumen
