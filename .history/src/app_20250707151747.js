...
const encuestaFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    if ((prevEncuesta && prevEncuesta !== encuestaActual) || finalizadaPrev) {
      await state.update({
        finalizada: false,
        preguntas:  null,
        respuestas: [],
    ...
    // 4) Guardar en memoria idEncuesta + idEmpresa (ahora desde preguntas)
    await state.update({
    ...
    // 7) Si no estaba en progreso, lo marco (idEmpresa desde preguntas)
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual,
    ...
    // 8) Si ya tenemos preguntas en memoria, no reiniciamos
    if (await state.get('preguntas')) return;

    // 9) Inicializar localmente y lanzar primera pregunta
    await state.update({
      preguntas,
      respuestas: [],
      paso:       0,
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    // Validaciones según tipo de respuesta
    if (preguntaActual.tipoRespuesta === 'RANGO') {
      const v = parseInt(respuesta, 10);
      if (isNaN(v) || v < preguntaActual.rangoIni || v > preguntaActual.rangoFin) {
        await flowDynamic(
          `❌ Por favor responde un número entre ${preguntaActual.rangoIni} y ${preguntaActual.rangoFin}.`
        );
        return gotoFlow(encuestaFlow);
      }
    } else if (preguntaActual.tipoRespuesta === 'CONFIRMA') {
    ...
    // Preparar payload para guardar respuestas
