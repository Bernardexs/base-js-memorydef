...
const defaultFlow = addKeyword(afirmaciones) // disparador: alguien dice “Sí” (o cualquiera de tus afirmaciones)
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    const texto = ctx.body.trim().toUpperCase();
    const yaFinalizada = await state.get('finalizada');

    // 1) Si ya completó la encuesta, sólo respondemos y salimos
    if (yaFinalizada) {
      await flowDynamic('✅ Ya completaste esta encuesta. ¡Gracias de nuevo!');
      return;
    }

    // 2) Si dice “Sí”, arrancamos la encuesta; si dice “No”, cancelamos
    ...
    // 3) Cualquier otro texto le pedimos que responda sólo Sí o No
