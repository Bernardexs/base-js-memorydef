...
const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    // Guardar en memoria el id de encuesta y de empresa
    await state.update({
      idEncuesta,
      idEmpresa: usuario.idEmpresa
    });

    // 3) Verificar estado en BD (404 ⇒ "no iniciado")
    let estadoEncuesta = 'no iniciado';
    try {
      const resp = await axios.post('http://localhost:7003/verificar-estado', {
        idContacto: ctx.from,
    ...
    // 4) Si ya completó, avisar y salir
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 5) Si "no iniciado", marcarla como "en progreso"
    if (estadoEncuesta === 'no iniciado') {
      const idEmpresa = await state.get('idEmpresa');
      await axios.post('http://localhost:7003/marcar-en-progreso', {
        idContacto: ctx.from,
        idEncuesta,
        idEmpresa
      });
    ...
    // 6) Si ya tenemos preguntas en memoria, no reiniciamos
    if (await state.get('preguntas')) {
      return;
    }

    // 7) Inicializar estado local y lanzar primera pregunta
    await state.update({
      preguntas,
      respuestas: [],
      paso:       0,
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    // 8) Validaciones según tipo de pregunta
    if (preguntaActual.tipoRespuesta === 'RANGO') {
    ...
    // 9) Guardar respuesta y avanzar paso
    respuestas.push(respuesta);
    paso++;

    // 10) Si quedan preguntas, enviar siguiente
    if (paso < preguntas.length) {
    ...
    // 11) Última respuesta: cerrar encuesta
    await state.update({ finalizada: true, preguntas: null, respuestas: [], paso: null });

    // 12) Preparar y enviar payload completo
    ...
    try {
      // a) Guardar respuestas en backend
      await axios.post('http://localhost:7003/guardar-respuestas', payload);
      await flowDynamic('📩 Tus respuestas fueron enviadas exitosamente.');

      // b) Marcar completada en BD
      const idEmpresa = await state.get('idEmpresa');
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: preguntas[0].idEncuesta,
