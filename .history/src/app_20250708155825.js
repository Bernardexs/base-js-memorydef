...
const encuestaFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    // 3) Verificar estado de la encuesta por contacto
    const encuestaId = preguntas[0].idEncuesta;
    const contactoId = ctx.from;

    // Verifica si el contacto ya tiene una encuesta y si ya la completó
    const estadoContacto = estadoEncuestas[contactoId] || {};
    const encuestaGuardada = estadoContacto[encuestaId];
    ...
    // Guardar el estado inicial de la encuesta para este contacto
    if (!estadoEncuestas[contactoId]) {
      estadoEncuestas[contactoId] = {};
    }

    estadoEncuestas[contactoId][encuestaId] = {
      estado: 'en progreso',
      idEmpresa: preguntas[0].idEmpresa,
      respuestas: []
    };
    ...
    // 5) Inicializar y lanzar primera pregunta
    await state.update({
    ...
    // Enviar el mensaje inicial de la encuesta
    await flowDynamic(`✅ ¡Hola ${usuario.nombre}! Empecemos con tu encuesta.`);
    const p0 = preguntas[0];
