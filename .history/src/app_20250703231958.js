...
const encuestaFlow = addKeyword(afirmaciones)
  .addAction(async (ctx, { state, flowDynamic }) => {
    console.log("🔍 Iniciando encuesta, verificando estado...");
    const datos = await state.getMyState();
    if (datos?.encuestaTerminada) {
      console.log(`ℹ️ El usuario ${ctx.from} ya terminó su encuesta. Ignorar reinicio.`);
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias de nuevo!');
      return;
    }

    // Verificamos si ya hay una encuesta activa
    if (datos?.preguntas) {
      console.log(`ℹ️ El usuario ${ctx.from} ya tiene una encuesta activa. Ignorar nueva coincidencia.`);
      await flowDynamic('✅ Ya estás participando en esta encuesta.');
      return;
    }
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    if (paso >= preguntas.length) {
      // Verificamos si ya llegamos a la última pregunta
...
const defaultFlow = addKeyword(EVENTS.WELCOME).addAction(async (ctx, { flowDynamic, state }) => {
  ...
  if (!usuario) {
   ...
   // 1. Consultar el estado de la encuesta
  const estadoEncuesta = await axios.post('http://localhost:7003/verificar-estado', {
    idContacto: ctx.from,
    idEncuesta: usuario.idEncuesta
  });

  if (estadoEncuesta.data.estadoEncuesta === 'completado') {
    // 2. Si la encuesta ya está completada, informar al usuario
