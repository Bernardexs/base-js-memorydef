...
const encuestaFlow = addKeyword(EVENTS.ACTION)
  ...
  const encuestaFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic }) => {
    // ⚠️ Si ya finalizó en esta sesión, no hacer nada
    ...
    if (prevEncuesta && prevEncuesta !== encuestaId) {
      // Si cambió la encuesta, reseteamos preguntas para volver a iniciar
      await state.update({ preguntas: null, respuestas: [], paso: null, finalizada: false });
    }

    // 4) Guardar en memoria idEncuesta e idEmpresa (solo lo sacamos de preguntas)
    await state.update({
      idEncuesta: encuestaId,
      idEmpresa: preguntas[0].idEmpresa
    });
    ...
    // 7) Si no estaba "en progreso", la marcamos ahora
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta,
    ...
    // 8) Si ya tenemos preguntas en memoria, salgo sin reenviar saludo
    if (await state.get('preguntas')) return;
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    // Si hay más preguntas, enviamos la siguiente
    if (paso < preguntas.length) {
      const s = preguntas[paso];
      let msg = `${paso+1}⃣ ${s.pregunta}`;
      if (s.textoIni && s.tipoRespuesta === 'RANGO') {
