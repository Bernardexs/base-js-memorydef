...
const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    if (prevEncuesta && prevEncuesta !== encuestaActual) {
      // si cambia de encuesta, resetear estado local
      await state.update({
        finalizada: false,
        preguntas:  null,
        respuestas: [],
    ...
    // guardar idEncuesta + idEmpresa en memoria
    await state.update({
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        // primera vez: marcamos "en progreso"
        await axios.post('http://localhost:7003/marcar-como-completada', {
          idContacto: ctx.from,
          idEncuesta: encuestaActual,
          idEmpresa:  usuario.idEmpresa
    ...
    // 4) Si ya completó, informar y salir
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 5) Asegurar que esté siempre "en progreso"
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual,
        idEmpresa:  usuario.idEmpresa
      });
    ...
    // 6) Si ya cargó preguntas en memoria, no reiniciar nada
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    // Última respuesta: cerrar encuesta localmente
    await state.update({ finalizada: true, preguntas: null, respuestas: [], paso: null });
    ...
    try {
      ...
      const empresa = await state.get('idEmpresa');
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: preguntas[0].idEncuesta,
        idEmpresa:  empresa
