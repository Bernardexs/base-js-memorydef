...
const encuestaFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    console.log('Estado actualizado:', await state.getMyState());

    // 4) Guardar en memoria idEncuesta + idEmpresa
    ...
    if (estadoEncuesta !== 'en progreso') {
      ...
      console.log('Encuesta marcada como completada en la base de datos');
    }
    ...
    console.log(`Enviando saludo: ¡Hola ${usuario.nombre}! Empecemos con tu encuesta.`);
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    console.log(`Respuesta recibida del usuario ${ctx.from}: ${respuesta}`);
