...
const encuestaFlow = addKeyword(EVENTS.ACTION)
 ...
 const encuestaFlow = addKeyword(EVENTS.ACTION)
  // 1) Acción inicial: enviamos un botón para arrancar
  .addAction(async (ctx, { state, flowDynamic }) => {
    clearReminder(ctx.from, PRE_ENCUESTA);
    ...
    // 3) Detectar si cambió la encuesta o si ya finalizó
    const encuestaId = preguntas[0].idEncuesta;
    const prevEncuesta = await state.get('idEncuesta');
    const finalizadaPrev = await state.get('finalizada');
    if ((prevEncuesta && prevEncuesta !== encuestaId) || finalizadaPrev) {
    ...
    // 4) Guardar idEncuesta e idEmpresa en memoria
    await state.update({
      idEncuesta: encuestaId,
      idEmpresa: preguntas[0].idEmpresa
    });

    // 5) Verificar estado en BD (404 ⇒ no iniciado)
    let estadoEncuesta = 'no iniciado';
    try {
      const resp = await axios.post('http://localhost:7003/verificar-estado', {
        idContacto: ctx.from,
    ...
    // 6) Si ya completó, notificamos y salimos
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 7) Si no estaba “en progreso”, lo marcamos
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaId,
    ...
    // 8) Si ya tenemos preguntas en memoria, salimos (ya arrancó antes)
    if (await state.get('preguntas')) return;

    // 9) Aquí enviamos botones para que el usuario confirme por acción
    await state.update({
      nombre: usuario.nombre,
      despedida: saludos[0]?.saludo3 || '✅ Gracias por participar en la encuesta.'
    });
    await flowDynamic({
      type: 'button',
      header: { text: 'Encuesta' },
      body: `✅ ¡Hola ${usuario.nombre}! ¿Estás list@ para iniciar la encuesta?`,
      action: {
        buttons: [
  ...
  // 2) Captura de la respuesta al botón
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    const texto = ctx.body.trim().toUpperCase();
    if (texto === 'NO') {
      await state.update({ finalizada: true });
      return flowDynamic('✅ Gracias por tu tiempo. Si deseas participar en otro momento, avísame.');
    }
    if (texto !== 'SI') {
      return flowDynamic('Por favor toca “Sí” o “No”.');
    }

    // El usuario tocó “Sí”, arrancamos la encuesta:
    const preguntas = (await state.get('preguntas')) || (await axios.get('http://localhost:7003/datos-encuesta')).data.preguntas;
    const usuarioNombre = await state.get('nombre');
    await state.update({
      preguntas,
      respuestas: [],
      paso: 0,
    ...
    await flowDynamic(`✅ ¡Genial! Empecemos con tu encuesta, ${usuarioNombre}.`);
    const p0 = preguntas[0];
    let msg = `1⃣ ${p0.pregunta}`;
    if (p0.textoIni && p0.tipoRespuesta === 'RANGO') {
      msg += `\n*Califica del rango ${p0.rangoIni} al ${p0.rangoFin}*`;
  ...
  // 3) Resto del flujo: respuestas pregunta a pregunta
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    clearReminder(ctx.from);
    const datos = await state.getMyState();
    if (!datos?.preguntas) return;
    ...
    if (preguntaActual.tipoRespuesta === 'RANGO') {
      const v = parseInt(respuesta, 10);
      if (isNaN(v) || v < preguntaActual.rangoIni || v > preguntaActual.rangoFin) {
        await flowDynamic(`❌ Responde un número entre ${preguntaActual.rangoIni} y ${preguntaActual.rangoFin}.`);
        return gotoFlow(encuestaFlow);
      }
    } else if (preguntaActual.tipoRespuesta === 'CONFIRMA') {
      if (!['SI','NO','SÍ'].includes(respuesta.toUpperCase())) {
...
/*const defaultFlow = addKeyword(afirmaciones)
 ...
 const defaultFlow = addKeyword(afirmaciones)
  .addAction(async (ctx, { state, gotoFlow, flowDynamic }) => {
    // 0) Si ya completó, avisamos y no hacemos nada más
    if (await state.get('finalizada')) {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 1) Si estamos en medio de la encuesta, reenviamos
    if (await state.get('preguntas')) {
      return gotoFlow(encuestaFlow);
    }

    // 2) Si no, arrancamos nueva encuesta
    await flowDynamic('¡Genial! Vamos a iniciar la encuesta…');
    return gotoFlow(encuestaFlow);
  })
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic }) => {
    // 0) Si ya completó, avisamos
    if (await state.get('finalizada')) {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }
    // Cualquier otra cosa le decimos que conteste Sí o No
    await flowDynamic('Por favor responde sólo Sí o No.');
  });
