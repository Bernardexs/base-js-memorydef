...
const encuestaFlow = addKeyword(afirmaciones)
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    console.log("📂 Datos cargados del estado:", datos);

    // ✅ NUEVA VALIDACIÓN: Si ya hay una encuesta activa, no hacer nada
    if (datos?.preguntas) {
      console.log(`ℹ️ El usuario ${ctx.from} ya tiene una encuesta activa. Ignorar nueva coincidencia.`);
      return; // No enviar mensaje, simplemente ignorar
    }

    // ✅ NUEVA VALIDACIÓN: Si estamos en PRE_ENCUESTA, proceder con la encuesta
    if (datos?.paso === PRE_ENCUESTA) {
      console.log(`✅ Usuario ${ctx.from} confirmó participación. Iniciando encuesta...`);
      ...
      if (!usuario) {
        console.log(`❌ No se encontró una encuesta asignada para el usuario ${ctx.from}.`);
        await flowDynamic('❌ No se encontró una encuesta asignada para ti.');
        return;
      }

      console.log(`👋 Hola ${usuario.nombre}, comenzando encuesta...`);
      ...
      console.log("📲 Enviando mensaje inicial:", msg0);
      await flowDynamic(msg0);
      scheduleReminder(ctx.from, 0, state);
      return;
    }

    // ✅ Si llegamos aquí, significa que el usuario dijo "SÍ" pero no está en PRE_ENCUESTA
    // Esto puede pasar si el usuario dice "SÍ" sin contexto
    console.log(`⚠️ Usuario ${ctx.from} dijo SÍ pero no está en contexto de encuesta`);
    return; // No hacer nada, ignorar
  })
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    // ... resto del código del capture permanece igual
    ...
    if (paso >= preguntas.length) {
      ...
      return; // ← CORREGIDO: Evitar continuar el flujo
    }
