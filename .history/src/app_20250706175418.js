...
/*const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
 ...
 const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    const localFinalizada = await state.get('finalizada');
    if (localFinalizada) {
    ...
    // ——— AÑADIDO: 3) Detectar cambio de encuesta en estado y resetear memoria —————————
    const encuestaActual = usuario.idEncuesta;
    const prevEncuesta = await state.get('idEncuesta');
    if (prevEncuesta && prevEncuesta !== encuestaActual) {
      await state.update({
        finalizada: false,
        preguntas: null,
        respuestas: [],
    ...
    await state.update({ idEncuesta: encuestaActual });

    // 4) Verificar estado en BD (404 ⇒ "no iniciado")
    let estadoEncuesta = 'no iniciado';
    try {
      const resp = await axios.post('http://localhost:7003/verificar-estado', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual
      });
      estadoEncuesta = resp.data.estadoEncuesta;
    } catch (err) {
    ...
    // 5) Si ya completó en BD, avisar y salir
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste la encuesta. ¡Gracias!');
      return;
    }

    // 6) Si no estaba en progreso, marcarla ahora
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual,
    ...
    // 7) Si ya cargamos preguntas en memoria, salimos
    const yaInit = await state.get('preguntas');
    if (yaInit) return;

    // 8) Inicializar estado local y lanzar primera pregunta
    await state.update({
      preguntas,
      respuestas: [],
      paso: 0,
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    // Limpiar recordatorio pendiente
    clearReminder(ctx.from);
    ...
    // ... aquí va tu validación por tipo RANGO / CONFIRMA ...

    // Guardar respuesta y avanzar paso
    respuestas.push(respuesta);
    paso++;
    ...
    // ——— AÑADIDO: 9) Al responder última, marcamos finalizada en memoria —————————
    await state.update({ finalizada: true, preguntas: null, respuestas: [], paso: null });

    // 10) Guardar en backend y marcar completada en BD
    ...
    // 11) Despedida y resumen (sin volver a gotoFlow)
