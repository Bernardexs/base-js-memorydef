...
const encuestaFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    // Verificar el id de la encuesta actual
    ...
    // Si el contacto ya completó esta encuesta, no le permitimos continuar
    if (finalizadaPrev && prevEncuesta === encuestaId) {
      await flowDynamic('✅ Ya completaste esta encuesta. ¡Gracias!');
      return;
    }

    // Si la encuesta ha cambiado, reiniciar el flujo para esa encuesta
    if (prevEncuesta && prevEncuesta !== encuestaId) {
      await state.update({ finalizada: false, preguntas: null, respuestas: [], paso: null });
    }

    // Guardar en memoria el idEncuesta y idEmpresa para este contacto
    await state.update({ idEncuesta: encuestaId, idEmpresa: preguntas[0].idEmpresa });

    // Verificar estado de la encuesta en la base de datos
    let estadoEncuesta = 'no iniciado';
    try {
      const resp = await axios.post('http://localhost:7003/verificar-estado', {
        idContacto: ctx.from,
      ...
      // Si no hay estado, asumimos que la encuesta no ha sido iniciada
    }

    // Si la encuesta ya está completada, mostrar mensaje
    if (estadoEncuesta === 'completado') {
      await flowDynamic('✅ Ya completaste esta encuesta. ¡Gracias!');
      return;
    }

    // Si la encuesta no está en progreso, la marcamos como iniciada
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaId,
    ...
    // Si ya tenemos preguntas, no reiniciar el flujo
    if (await state.get('preguntas')) return;

    // Inicializar preguntas y comenzar la encuesta
    await state.update({
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    if (paso < preguntas.length) {
      ...
      if (siguiente.textoIni && siguiente.tipoRespuesta === 'RANGO') {
        msg += `\n*Califica del rango ${siguiente.rangoIni} al ${siguiente.rangoFin}*`;
        msg += '\n' + siguiente.textoIni.split('=').map(x => x.replace('-', ' - ').trim()).join('\n');
      } else if (siguiente.textoIni) {
        msg += '\n' + siguiente.textoIni.split('=').map(x => x.replace('-', ' - ').trim()).join('\n');
      }
      await state.update({ preguntas, respuestas, paso, despedida });
    ...
    // Si ya se completaron todas las preguntas
    await state.update({ finalizada: true, preguntas: null, respuestas: [], paso: null });
    ...
    try {
      ...
      // Marcar completada la encuesta en la base de datos
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta,
        idEmpresa
...
const defaultFlow = addKeyword(afirmaciones)
  .addAction(async (ctx, { state, flowDynamic, gotoFlow }) => {
    // 1) Verificar si ya completó la encuesta en esta sesión
    const yaFinalizada = await state.get('finalizada');
    const idEncuestaActual = await state.get('idEncuesta');  // Obtener idEncuesta guardado
    if (yaFinalizada && idEncuestaActual) {
      // Si la encuesta está finalizada y ya tiene idEncuesta, no permitir que reinicie
      await flowDynamic('✅ Ya completaste esta encuesta. ¡Gracias de nuevo!');
      return;
    }

    // 2) Si no está finalizada o no tiene idEncuesta guardado, reiniciamos la encuesta
