 ...
 const encuestaFlow = addKeyword(EVENTS.ACTION) // NO usa keywords
  .addAction(async (ctx, { state, flowDynamic }) => {
    ...
    try {
      const resp = await axios.post('http://localhost:7003/verificar-estado', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual      // ← uso correcto
      });
      estadoEncuesta = resp.data.estadoEncuesta;
    } catch (err) {
      if (!(err.response && err.response.status === 404)) {
    ...
    // 5) Si no estaba "en progreso", marcarla ahora
    if (estadoEncuesta !== 'en progreso') {
      await axios.post('http://localhost:7003/marcar-como-completada', {
        idContacto: ctx.from,
        idEncuesta: encuestaActual,     // ← uso correcto
        idEmpresa:  empresaActual       // ← obtenida arriba
      });
    }

    // 6) Si ya tenemos preguntas en memoria, no reiniciamos nada
    if (await state.get('preguntas')) return;

    // 7) Inicializar localmente y lanzar la primera pregunta
    await state.update({
      preguntas,
      respuestas: [],
      paso:       0,
  ...
  .addAnswer(null, { capture: true }, async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    try {
      ...
      // 2) Marcar completada en BD usando el mismo empresaActual
